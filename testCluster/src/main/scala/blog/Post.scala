...
class Post(authorListing: ActorRef) extends Actor with ActorLogging {

  import Post._

  println(s"Post actor start")
...
//  override def receiveCommand: Receive = initial
  ...
  def initial: Receive = {
    ...
    case AddPost(_, content) =>
      ...
      //        }
      {
        state = state.updated(PostAdded(content))
        context.become(created)
        log.info("New post saved: {}", state.content.title)
      }
...
//      }
      ...
      state = state.updated(BodyChanged(body))
      log.info("Post changed: {}", state.content.title)
